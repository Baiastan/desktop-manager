...
const Timer = ({ deadline, title, dateCreated }) => {
  const [hoursLeft, setHoursLeft, calculateHours] = useCalculateDeadline(
    'hours',
    deadline
  );
  const [minutes, setMinutes, calculateMin] = useCalculateDeadline(
    'minutes',
    deadline
  );
  const [totalHours] = useCalculateDeadline(
    'totalHours',
    deadline,
    dateCreated
  ); //need to fix it //need to save in server database
